...
%% Must add ones(m,1) to create the bias term
...
%% size(yVec) - 5000 by 10

%% size(sum(-1 * yVec .* log(h) - (1-yVec) .* log(1-h))) 1 by 10 

%% size(sum(sum(-1 * yVec .* log(h) - (1-yVec) .* log(1-h)))) 1 by 1

cost = (1/m)*sum(sum(-1 * yVec .* log(h) - (1-yVec) .* log(1-h)));

% Explictly skip the first value of j
